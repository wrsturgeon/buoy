...
void lcd_draw_char(uint8_t x, uint8_t y, uint16_t character, uint16_t fColor, uint16_t bColor) {
  ...
  for (i = 0; i < 5; i++) {
    ...
    for (j = 0; j < 8; j++) {
      if (((pixels >> j) & 1) == 1) {
        lcd_draw_pixel(x + i, y + j, fColor);
      } else {
        lcd_draw_pixel(x + i, y + j, bColor);
      }
    }
  }
  //	}
...
void circle_remainder_recursive(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint16_t rsq, uint16_t color) {
  ...
  lcd_draw_block(x1, y1, x1 + l, y1 + l, color);
  lcd_draw_block(x1, y0 - dy - l, x1 + l, y0 - dy, color);
  lcd_draw_block(x0 - dx - l, y1, x0 - dx, y1 + l, color);
  lcd_draw_block(x0 - dx - l, y0 - dy - l, x0 - dx, y0 - dy, color);
  lcd_draw_block(x0 + dy, y0 + dx, x0 + dy + l, y0 + dx + l, color);
  lcd_draw_block(x0 + dy, y0 - dx - l, x0 + dy + l, y0 - dx, color);
  lcd_draw_block(x0 - dy - l, y0 + dx, x0 - dy, y0 + dx + l, color);
  lcd_draw_block(x0 - dy - l, y0 - dx - l, x0 - dy, y0 - dx, color);
...
void lcd_draw_circle(uint8_t x0, uint8_t y0, uint8_t radius, uint16_t color) {
  ...
  uint8_t y1 = y0 + hsl;
  lcd_draw_block(x0 - hsl, y0 - hsl, x0 + hsl, y1, color);
...
void lcd_set_screen(uint16_t color) {
  lcd_draw_block(0, 0, 159, 127, color);
}
