...
build/%: $(shell find main -type f) includes.txt
	...
	find -X ${HOME}/esp/esp-idf/components -type d -name include ! -path '*esp32c2*' ! -path '*esp32c3*' ! -path '*esp32c6*' ! -path '*esp32h2*' ! -path '*esp32h4*' ! -path '*esp32s2*' ! -path '*esp32s3*' | xargs -I {} echo '        "{}",' >> .vscode/c_cpp_properties.json
	find -X ${HOME}/esp/esp-idf/components -type d -name esp32 -path '*/include/esp32' | xargs -I {} echo '        "{}",' >> .vscode/c_cpp_properties.json
